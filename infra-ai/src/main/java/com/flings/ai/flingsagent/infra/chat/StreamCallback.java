...
public interface StreamCallback {
    ...
    /**
     * 接收一次智能体编排事件（多智能体链路专用）
     * <p>
     * 用于把 Planner 计划、worker 状态、Critic 反思等「非答案内容」推送到前端做协作可视化，
     * 与 onContent 的最终答案流互不干扰。默认空实现，非多智能体场景可忽略
     *
     * @param eventType SSE 事件名（如 plan / agent_status / reflection）
     * @param payload   事件载荷（由上层序列化为 JSON）
     */
    default void onAgentEvent(String eventType, Object payload) {
    }
}
