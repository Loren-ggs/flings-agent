...
export function WelcomeScreen() {
  ...
  return (
    <div className="relative flex min-h-full items-center justify-center overflow-hidden px-4 py-16 sm:px-6">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 bg-gradient-to-br from-[#F8FAFC] via-white to-[#FFF7ED]"
      />
      <div
        aria-hidden="true"
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -top-32 right-[-40px] h-72 w-72 rounded-full bg-gradient-radial from-[#FED7AA]/60 via-transparent to-transparent blur-3xl animate-float"
      />
      <div
        aria-hidden="true"
      ...
      <div className="relative w-full max-w-[860px]">
        ...
        >
          <span className="inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/70 px-3 py-1 text-xs font-medium text-[#EA580C] shadow-sm">
        ...
        >
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              ...
              >
                <span className="inline-flex items-center gap-2">
                  <Brain className={cn("h-3.5 w-3.5", deepThinkingEnabled && "text-[#F97316]")} />
                  深度思考
                  {deepThinkingEnabled ? (
          ...
          {deepThinkingEnabled ? (
            <p className="mt-3 text-xs text-[#EA580C]">
            ...
            {promptPresets.map((preset) => {
              ...
              return (
                <button
                  ...
                  disabled={isStreaming}
                  className={cn(
                    "group rounded-2xl border border-white/70 bg-white/70 p-4 text-left shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-[#FED7AA] hover:shadow-md",
                    isStreaming && "cursor-not-allowed opacity-60"
                  )}
                >
                  <div className="flex items-center gap-3">
                    <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#FFF7ED] text-[#EA580C]">
                  ...
                  <div className="mt-3 flex items-center gap-2 text-xs text-[#94A3B8]">
                    ...
                    <ArrowUpRight className="h-3.5 w-3.5 text-[#FDBA74] transition-colors group-hover:text-[#F97316]" />
                  </div>
                </button>
              );
            })}
