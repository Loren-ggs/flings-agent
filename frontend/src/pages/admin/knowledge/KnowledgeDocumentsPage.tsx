...
const statusDotClass = (status?: string | null) => {
  ...
  if (normalized === "pending") return "bg-stone-400";
...
const FILE_TYPE_MAP: Record<string, { icon: typeof FileText; color: string }> = {
  ...
  markdown:    { icon: FileText, color: "text-orange-500" },
  md:          { icon: FileText, color: "text-orange-500" },
  doc:         { icon: FileText, color: "text-orange-600" },
  docx:        { icon: FileText, color: "text-orange-600" },
...
const renderFileTypeIcon = (fileType?: string | null, sourceType?: string | null) => {
  ...
  return <FileText className="h-4 w-4 shrink-0 text-stone-400" />;
};
    ...
    <div className="admin-page">
      ...
      <Card>
        ...
        <CardContent>
                      ...
                      <div className="flex items-center gap-2.5 min-w-0 max-w-[320px]">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5 min-w-0">
                            <button
                              type="button"
                              className="block truncate min-w-0 text-left font-medium text-stone-900 transition-colors hover:text-orange-600 hover:underline underline-offset-4"
                              title={doc.docName || ""}
          ...
          {pageData ? (
            <div className="mt-4 flex flex-wrap items-center justify-between gap-2 text-sm text-stone-500">
                      ...
                      <div>
                        ...
                        <div className="flex h-9 items-center">
                          <button
                            ...
                            className={cn(
                              "relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background",
                              detailNoChunk ? "bg-orange-600" : "bg-stone-200"
                            )}
                          >
                            <span
                              className={cn(
            ...
            (() => {
              ...
              return (
                <div className="flex-1 overflow-y-auto sidebar-scroll">
                  {head ? (
                    <pre className="mx-6 mt-4 overflow-auto rounded-lg border bg-stone-50 px-4 py-3 font-mono text-xs leading-relaxed text-stone-600 dark:border-stone-800 dark:bg-stone-950 dark:text-stone-400">
                      {head}
                    </pre>
                  ) : null}
              ...
              {logData.records.slice(0, 1).map((log) => {
                ...
                <div key={log.id} className="space-y-4">
                  ...
                  <div className={cn("grid gap-3", isPipelineLog ? "grid-cols-2 md:grid-cols-3" : "grid-cols-2 md:grid-cols-3 lg:grid-cols-4")}>
                    {!isPipelineLog && (
                      <div className="rounded-lg border bg-stone-50/50 p-3">
                  ...
                  <div className="flex items-center gap-2 text-sm text-stone-500">
                    <span>执行时间</span>
                    <span className="tabular-nums text-stone-700">{formatFullDateTime(log.startTime)}</span>
                    <span>~</span>
                    <span className="tabular-nums text-stone-700">{log.endTime ? formatFullDateTime(log.endTime) : "进行中"}</span>
                  </div>
      ...
      {selectedIds.size > 0 && (
        <div className="fixed inset-x-0 bottom-6 z-50 flex justify-center">
          <div className="animate-fade-up rounded-2xl bg-stone-900 px-5 py-3 text-sm text-white shadow-[0_10px_40px_rgba(0,0,0,0.15)]">
            <div className="flex items-center gap-3">
                      ...
                      <FormControl>
                        <div className="flex h-9 items-center">
                          <button
                            ...
                            className={cn(
                              "relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background",
                              noChunk ? "bg-orange-600" : "bg-stone-200"
                            )}
                          >
                            <span
                              className={cn(
