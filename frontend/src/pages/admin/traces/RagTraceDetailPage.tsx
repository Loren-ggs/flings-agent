...
const STATUS_COLORS: Record<StatusType, { dot: string; bar: string }> = {
  ...
  default: { dot: "bg-stone-300", bar: "bg-stone-300" }
};
  ...
  return (
      <div
          ...
          className={cn(
              ...
              isRoot && "bg-orange-50/40 border-b border-orange-100",
              isTopSlowest && !isSelected && !isRoot && "bg-amber-50/40",
              isSelected && "bg-orange-50/60 ring-1 ring-inset ring-orange-200"
          )}
      >
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span
              className={cn(
                  "truncate",
                  isRoot ? "text-sm font-semibold text-orange-900" : "text-sm text-stone-700"
              )}
        ...
        <div className="flex items-center">
          <div className="relative w-full h-6 bg-stone-50 rounded overflow-hidden">
            {[25, 50, 75].map(p => (
                <div
                    key={p}
  ...
  return (
      <Card>
        <CardHeader className="py-3 px-4">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <CardTitle className="text-sm font-medium text-stone-700 truncate" title={displayName}>
                {displayName}
              </CardTitle>
        ...
        <CardContent className="px-4 pb-4 pt-0 space-y-3">
          ...
          {node.extraData && (
              <div>
                <p className="text-xs font-medium text-stone-500 mb-1">额外数据</p>
                <pre className="text-xs bg-stone-50 border border-stone-200 rounded p-2 overflow-x-auto whitespace-pre-wrap break-all text-stone-700">
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        <span
            className={cn(
                ...
                mono && "font-mono",
                highlightClass,
                copyable && "cursor-pointer hover:text-orange-600 transition-colors"
            )}
            title={value}
        ...
        {copyable && (
            <Copy className="h-3 w-3 text-stone-300 shrink-0" />
        )}
      </div>
  );
}
...
export function RagTraceDetailPage() {
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-sm">
              <Link to="/admin/traces" className="text-stone-500 hover:text-stone-700">
                链路追踪
              </Link>
              <span className="text-stone-300">/</span>
              <span className="text-stone-400">详情</span>
            </div>
            <Button
                asChild
                variant="outline"
          ...
          <div className="min-h-[300px] flex items-center justify-center">
            <div className="text-center text-stone-500">
              <AlertTriangle className="h-12 w-12 mx-auto mb-4 text-stone-300" />
  ...
  return (
      <div className="space-y-4 pb-8">
        ...
        <span
            className="font-mono cursor-pointer hover:text-stone-700 flex items-center gap-1 transition-colors"
        ...
        {/* 指标条 */}
        <div className="flex items-center bg-stone-50 rounded-lg border border-stone-200 divide-x divide-stone-200">
          <MetricItem
              icon={Clock}
              label="总耗时"
        ...
        <Card>
          ...
          <CardContent className="p-0">
            {timeline.nodes.length === 0 ? (
                ...
                <div>
                  <div className="grid grid-cols-[minmax(180px,1fr)_120px_2fr_100px] gap-4 px-4 py-2 text-xs font-medium text-stone-500 bg-stone-50 border-y border-stone-100">
                    <span>节点</span>
                    <span>类型</span>
                    <span>时间线</span>
