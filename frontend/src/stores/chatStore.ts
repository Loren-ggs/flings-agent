...
/**
 * 合并 agent 状态事件：running 时追加卡片，完成态覆盖最近一个同角色仍在 running 的卡片
 * （反思多轮下同一角色会多次出现，按出现顺序保留）
 */
function mergeAgentStatus(agents: AgentStatusInfo[], next: AgentStatusInfo): AgentStatusInfo[] {
  if (next.status === "running") {
    if (agents.some((agent) => agent.role === next.role && agent.status === "running")) {
      return agents;
    }
    return [...agents, next];
  }
  for (let i = agents.length - 1; i >= 0; i -= 1) {
    if (agents[i].role === next.role && agents[i].status === "running") {
      const copy = [...agents];
      copy[i] = next;
      return copy;
    }
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  sendMessage: async (content) => {
    ...
    const updateAgentTrace = (
      messageId: string,
      updater: (trace: AgentTrace) => AgentTrace
    ) => {
      set((state) => ({
        messages: state.messages.map((message) => {
          if (message.id !== messageId) return message;
          const current: AgentTrace = message.agentTrace ?? { agents: [], reflections: [] };
          return { ...message, agentTrace: updater(current) };
        })
      }));
    };
    ...
    const handlers = {
      ...
      onEvent: (event: string, payload: unknown) => {
        if (get().streamingMessageId !== assistantId) return;
        if (event === "plan") {
          updateAgentTrace(assistantId, (trace) => ({ ...trace, plan: payload as AgentPlanInfo }));
        } else if (event === "agent_status") {
          updateAgentTrace(assistantId, (trace) => ({
            ...trace,
            agents: mergeAgentStatus(trace.agents, payload as AgentStatusInfo)
          }));
        } else if (event === "reflection") {
          updateAgentTrace(assistantId, (trace) => ({
            ...trace,
            reflections: [...trace.reflections, payload as AgentReflectionInfo]
          }));
        }
      }
    };
